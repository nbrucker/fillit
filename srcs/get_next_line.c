...
int		ft_cut(t_gnl **current, char **line)
{
	char	*tmp;
...
int		ft_read(t_gnl **current, int fd, char **line)
{
...
int		ft_make_lst(t_gnl **current, t_gnl **lst, int fd)
{
	t_gnl	*new;
...
int		get_next_line(const int fd, char **line)
{
	...
	if (fd < 0 || BUFF_SIZE < 1 || line == NULL || fd > OPEN_MAX)
		return (-1);
	current = lst;
	i = -1;
